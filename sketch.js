...
function setup() {
  ...
  wall = new Ground(750, 250, 50, 500);
...
function draw() {
  ...
  if(bird.x-wall.x<bird.width/2+wall.width/2 &&
    wall.x-bird.x<wall.width/2+bird.width/2 &&
    bird.y-wall.y<bird.height/2+wall.height/2 &&
    wall.y-bird.y<wall.height/2+bird.height/2) {
   gameState = 3;
   bird.fill("green");
   wall.fill("red");
 }
